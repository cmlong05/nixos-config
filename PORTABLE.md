...
- 现改为 **1 个 host（`os-disk/portable-chen`）+ 机器变体**：基础系统**硬件无关**，插到任何机器都能进桌面；
  每台机器是一个 `specialisation`（`chen-desktop` / `chen-laptop-amd` / `chen-laptop-intel`），
  指向 `machines/` 里的组件组合。换机器**零命令**，且所有变体每次 switch 一起重建、**永远同步**。
- 采用 **方案 A**（基础=硬件无关，而非完整 NVIDIA）：保证"未知机器插上必能进桌面"。
  代价是 AMD+NVIDIA 台式机默认走 nouveau，要手动选 `chen-desktop` 变体。
